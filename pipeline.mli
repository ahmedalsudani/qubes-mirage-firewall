...
(** [create ~max_in_flight] is a pipeline allowing at most [max_in_flight]
    operations in flight at once. Raises [Invalid_argument] if
    [max_in_flight < 1]. *)

val submit : t -> (unit -> unit Lwt.t) -> unit Lwt.t
(** [submit t fn] schedules [fn ()] to run in the background. The returned
    thread becomes determined as soon as there is room in the pipeline:
    immediately while fewer than [max_in_flight] operations are in flight,
    otherwise once an in-flight operation completes. The receive loop should
    bind on this thread, so that it keeps pulling frames while transmits are in
    flight but pauses when the pipeline is full. Exceptions raised by [fn] are
    caught and logged so they cannot escape into Lwt's async exception hook. *)
